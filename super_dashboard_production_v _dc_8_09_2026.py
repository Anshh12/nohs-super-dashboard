...
Integrates:
  ...
  4. Household Listing Dashboard (10 tabs)
  5. Data Cleaner (household merge + record redundancy cleanup)
...
def agreement_rollup(agreement_df, selected_field_count=0):
    """Return transparent field-level and observation-weighted agreement metrics."""
    assessable_fields = int(len(agreement_df))
    assessed_observations = int(agreement_df['Assessed'].sum()) if assessable_fields else 0
    total_agree = int(agreement_df['Agree'].sum()) if assessable_fields else 0
    return {
        'Fields Selected': int(selected_field_count),
        'Fields With Usable Data': assessable_fields,
        'Data Points Assessed': assessed_observations,
        'Avg Agreement %': round(float(agreement_df['Agreement %'].mean()), 1)
            if assessable_fields else 0,
        'Overall Agreement %': round(100 * total_agree / assessed_observations, 1)
            if assessed_observations else 0,
        'Min Agreement %': round(float(agreement_df['Agreement %'].min()), 1)
            if assessable_fields else 0,
    }
...
def build_examiner_reports(merged, compare_cols, selected_keys, label_a, label_b,
    ...
    for (exam_a, exam_b), group in grouped:
        ...
        pair_rollup = agreement_rollup(pair_agreement, len(compare_cols))
...
def build_examiner_aggregated_reports(merged, compare_cols, selected_keys, label_a, label_b,
    ...
    if 'examiner_number_A' in merged.columns:
        for exam, group in merged.groupby('examiner_number_A', dropna=False):
            ...
            ag_rollup = agreement_rollup(ag_df, len(compare_cols))
...
def get_unmatched(pt, other_pt, keys):
    ...
    display_columns = []
    for col in ['record_id'] + list(keys) + extra:
        if col in pt.columns and col not in display_columns:
            display_columns.append(col)
    return pt.loc[mask, display_columns].copy()
...
def cleaner_is_blank(value):
    if pd.isna(value):
        return True
    return str(value).strip() in {"", "nan", "NaN", "<NA>", "None", "NaT"}
...
def cleaner_normalized_cell(value):
    if cleaner_is_blank(value):
        return ""
...
def cleaner_nonblank_count(row, columns):
    return int(sum(not cleaner_is_blank(row.get(c, np.nan)) for c in columns))

def cleaner_first_nonblank(values):
    for value in values:
        if not cleaner_is_blank(value):
            return value
    return np.nan

def cleaner_distinct_display_values(values):
    seen = {}
    for value in values:
        if cleaner_is_blank(value):
            continue
        key = cleaner_normalized_cell(value)
        if key not in seen:
            seen[key] = cleaner_display_value(value)
    return list(seen.values())

def cleaner_detect_columns(df):
    cols = list(df.columns)
    record_col = find_col(["Record ID", "record_id"], cols) or (cols[0] if cols else None)
    repeat_col = find_col(["Repeat Instrument", "redcap_repeat_instrument", "repeat_instrument"], cols)
    instance_col = find_col(["Repeat Instance", "redcap_repeat_instance", "repeat_instance"], cols)
    cluster_col = find_col(["Cluster Code", "cluster_code", "cluster"], cols)
    household_col = find_col(["Household Number", "household_number"], cols)
    date_col = find_col(["Listing Date", "Consent date", "date"], cols)
    return {
...
def cleaner_add_household_keys(df, original_columns, record_col, repeat_col, cluster_col, household_col):
    keyed = df.copy()
    if record_col and record_col in keyed.columns:
        record_keys = keyed[record_col].map(cleaner_key_value)
    else:
        record_keys = pd.Series([""] * len(keyed), index=keyed.index)

    if repeat_col and repeat_col in keyed.columns:
        repeat_text = keyed[repeat_col].astype("string").str.strip().str.casefold()
        household_mask = keyed[repeat_col].map(cleaner_is_blank) | repeat_text.isin(["", "nan", "<na>"])
    else:
        household_mask = pd.Series([True] * len(keyed), index=keyed.index)

    cluster_keys = (
        keyed[cluster_col].map(cleaner_key_value)
        if cluster_col and cluster_col in keyed.columns
        else pd.Series([""] * len(keyed), index=keyed.index)
    )
    household_keys = (
        keyed[household_col].map(cleaner_key_value)
        if household_col and household_col in keyed.columns
        else pd.Series([""] * len(keyed), index=keyed.index)
    )

    record_to_cluster = {}
    record_to_household = {}
    for idx in keyed.index[household_mask]:
        rec_key = record_keys.loc[idx]
        if not rec_key:
            continue
        if cluster_keys.loc[idx]:
            record_to_cluster.setdefault(rec_key, cluster_keys.loc[idx])
        if household_keys.loc[idx]:
            record_to_household.setdefault(rec_key, household_keys.loc[idx])

    for idx in keyed.index:
        rec_key = record_keys.loc[idx]
        if rec_key:
            if not cluster_keys.loc[idx] and rec_key in record_to_cluster:
                cluster_keys.loc[idx] = record_to_cluster[rec_key]
            if not household_keys.loc[idx] and rec_key in record_to_household:
                household_keys.loc[idx] = record_to_household[rec_key]

    physical_keys = []
    has_household_key = []
    for idx in keyed.index:
        cluster_key = cluster_keys.loc[idx]
        household_key = household_keys.loc[idx]
        if cluster_key and household_key:
            physical_keys.append(f"C:{cluster_key}|H:{household_key}")
            has_household_key.append(True)
        else:
            fallback = record_keys.loc[idx] or f"ROW:{int(keyed.loc[idx, '__clean_source_order'])}"
            physical_keys.append(f"R:{fallback}")
            has_household_key.append(False)

    keyed["__clean_record_key"] = record_keys
    keyed["__clean_cluster_key"] = cluster_keys
    keyed["__clean_household_key"] = household_keys
    keyed["__clean_physical_key"] = physical_keys
    keyed["__clean_has_household_key"] = has_household_key
    keyed["__clean_is_household"] = household_mask
    return keyed

def cleaner_choose_household_row(rows, original_columns, strategy, date_col):
    candidates = rows.copy()
    candidates["__clean_score"] = candidates.apply(
        lambda row: cleaner_nonblank_count(row, original_columns), axis=1
    )
    if strategy == "Latest date, then most complete" and date_col and date_col in candidates.columns:
        candidates["__clean_date_sort"] = pd.to_datetime(candidates[date_col], errors="coerce")
        candidates = candidates.sort_values(
            ["__clean_date_sort", "__clean_score", "__clean_source_order"],
            ascending=[False, False, True],
            na_position="last",
        )
    elif strategy == "Earliest row in file":
        candidates = candidates.sort_values("__clean_source_order", ascending=True)
    else:
        candidates = candidates.sort_values(["__clean_score", "__clean_source_order"], ascending=[False, True])
    return candidates.iloc[0]

def clean_redundant_households(
    raw_df,
    record_col=None,
    repeat_col=None,
    instance_col=None,
    ...
    date_col=None,
    trim_text=True,
    remove_exact_rows=True,
    recompute_instances=True,
    household_strategy="Most complete household row",
):
    ...
    if raw_df.empty:
        return raw_df.copy(), empty_report

    detected = cleaner_detect_columns(raw_df)
    record_col = record_col or detected["record_col"]
    repeat_col = repeat_col or detected["repeat_col"]
    instance_col = instance_col or detected["instance_col"]
    cluster_col = cluster_col or detected["cluster_col"]
    household_col = household_col or detected["household_col"]
    ...
    if trim_text:
        ...
        for col in text_cols:
            working[col] = working[col].map(lambda value: value.strip() if isinstance(value, str) else value)
            working[col] = working[col].replace({"": np.nan})

    working["__clean_source_order"] = np.arange(len(working))
    working["__clean_source_row"] = working.index.to_series().astype(int) + 2

    exact_duplicate_rows = pd.DataFrame()
    if remove_exact_rows and original_columns:
        duplicate_mask = working.duplicated(subset=original_columns, keep="first")
        if duplicate_mask.any():
            exact_duplicate_rows = working.loc[duplicate_mask, ["__clean_source_row"] + original_columns].copy()
            exact_duplicate_rows.insert(0, "Cleaner Removal Reason", "Exact full-row duplicate")
            working = working.loc[~duplicate_mask].copy()

    keyed = cleaner_add_household_keys(
        working, original_columns, record_col, repeat_col, cluster_col, household_col
    )
    ...
    removed_records = []
    conflict_records = []
    duplicate_household_rows_merged = 0
    synthetic_households_created = 0
    repeat_duplicates_removed = 0

    group_order = (
        keyed.groupby("__clean_physical_key", sort=False)["__clean_source_order"]
        .min()
        .sort_values()
        .index
        .tolist()
    ...
    admin_repeat_exclusions = {record_col, instance_col, cluster_col, household_col}
    repeat_signature_cols = [c for c in original_columns if c not in admin_repeat_exclusions]

    for physical_key in group_order:
        group = keyed[keyed["__clean_physical_key"] == physical_key].sort_values("__clean_source_order")
        household_rows = group[group["__clean_is_household"]]
        repeat_rows = group[~group["__clean_is_household"]]
        original_record_ids = [
            cleaner_display_value(value)
            for value in group[record_col].tolist()
            if record_col and record_col in group.columns and not cleaner_is_blank(value)
        ]
        original_record_ids = list(dict.fromkeys(original_record_ids))

        if not household_rows.empty:
            canonical = cleaner_choose_household_row(
                household_rows, original_columns, household_strategy, date_col
            )
            canonical_record_id = canonical.get(record_col, np.nan) if record_col else np.nan
            if cleaner_is_blank(canonical_record_id):
                canonical_record_id = cleaner_first_nonblank(group[record_col]) if record_col in group.columns else np.nan
            combined = canonical[original_columns].copy()
            fill_order = household_rows.copy()
            fill_order["__clean_score"] = fill_order.apply(
                lambda row: cleaner_nonblank_count(row, original_columns), axis=1
            )
            fill_order = fill_order.sort_values(["__clean_score", "__clean_source_order"], ascending=[False, True])
            for col in original_columns:
                if col in {record_col, repeat_col, instance_col}:
                    continue
                if cleaner_is_blank(combined.get(col, np.nan)):
                    combined[col] = cleaner_first_nonblank(fill_order[col]) if col in fill_order.columns else np.nan

                distinct_values = cleaner_distinct_display_values(household_rows[col]) if col in household_rows.columns else []
                if len(distinct_values) > 1:
                    kept = cleaner_display_value(combined.get(col, np.nan))
                    discarded = [v for v in distinct_values if cleaner_normalized_cell(v) != cleaner_normalized_cell(kept)]
                    if discarded:
                        conflict_records.append({
                            "Physical Household Key": physical_key,
                            "Canonical Record ID": cleaner_display_value(canonical_record_id),
                            "Column": col,
                            "Kept Value": kept,
                            "Other Value(s)": "; ".join(discarded[:10]),
                        })
            duplicate_household_rows_merged += max(0, len(household_rows) - 1)
        else:
            canonical_record_id = cleaner_first_nonblank(group[record_col]) if record_col and record_col in group.columns else np.nan
            combined = pd.Series({col: np.nan for col in original_columns})
            if keyed.loc[group.index[0], "__clean_has_household_key"]:
                synthetic_households_created += 1
                if record_col:
                    combined[record_col] = canonical_record_id
                if cluster_col and cluster_col in original_columns:
                    combined[cluster_col] = cleaner_first_nonblank(group[cluster_col])
                if household_col and household_col in original_columns:
                    combined[household_col] = cleaner_first_nonblank(group[household_col])
                if repeat_col and repeat_col in original_columns:
                    combined[repeat_col] = np.nan
                if instance_col and instance_col in original_columns:
                    combined[instance_col] = np.nan
            else:
                combined = None

        if record_col and not cleaner_is_blank(canonical_record_id):
            if combined is not None:
                combined[record_col] = canonical_record_id
            repeat_rows = repeat_rows.copy()
            if not repeat_rows.empty:
                repeat_rows[record_col] = canonical_record_id

        kept_repeat_rows = []
        seen_repeat_signatures = set()
        for _, repeat_row in repeat_rows.iterrows():
            signature = tuple(cleaner_normalized_cell(repeat_row.get(col, np.nan)) for col in repeat_signature_cols)
            if remove_exact_rows and signature in seen_repeat_signatures:
                repeat_duplicates_removed += 1
                removed = {col: repeat_row.get(col, np.nan) for col in original_columns}
                removed["Cleaner Removal Reason"] = "Duplicate repeated row after household merge"
                removed["Source CSV Row"] = int(repeat_row.get("__clean_source_row", 0))
                removed["Canonical Record ID"] = cleaner_display_value(canonical_record_id)
                removed["Physical Household Key"] = physical_key
                removed_records.append(removed)
                continue
            seen_repeat_signatures.add(signature)
            kept_repeat_rows.append(repeat_row[original_columns].copy())

        if combined is not None:
            output_rows.append(combined)
        output_rows.extend(kept_repeat_rows)

        cluster_display = ""
        household_display = ""
        if group["__clean_has_household_key"].any():
            cluster_display = cleaner_first_nonblank(group[cluster_col]) if cluster_col and cluster_col in group.columns else ""
            household_display = cleaner_first_nonblank(group[household_col]) if household_col and household_col in group.columns else ""

        action = "Merged duplicate household records" if len(original_record_ids) > 1 or len(household_rows) > 1 else "Kept"
        if household_rows.empty and combined is not None:
            action = "Created one household row from repeated rows"
        elif not group["__clean_has_household_key"].any():
            action = "No household key; kept by record"

        merge_records.append({
            "Physical Household Key": physical_key,
            "Cluster": cleaner_display_value(cluster_display),
            "Household Number": cleaner_display_value(household_display),
            "Canonical Record ID": cleaner_display_value(canonical_record_id),
            "Original Record IDs": ", ".join(original_record_ids),
            "Household Rows Before": len(household_rows),
            "Repeat Rows Before": len(repeat_rows),
            "Repeat Rows Kept": len(kept_repeat_rows),
            "Repeat Duplicates Removed": len(repeat_rows) - len(kept_repeat_rows),
            "Action": action,
        })

    cleaned_df = pd.DataFrame(output_rows, columns=original_columns)
    if recompute_instances and repeat_col and instance_col and repeat_col in cleaned_df.columns and instance_col in cleaned_df.columns:
        repeat_text = cleaned_df[repeat_col].astype("string").str.strip().str.casefold()
        repeat_mask = cleaned_df[repeat_col].notna() & ~repeat_text.isin(["", "nan", "<na>"])
        grouping_cols = [record_col, repeat_col] if record_col and record_col in cleaned_df.columns else [repeat_col]
        for _, idxs in cleaned_df.loc[repeat_mask].groupby(grouping_cols, sort=False).groups.items():
            cleaned_df.loc[list(idxs), instance_col] = range(1, len(idxs) + 1)

    removed_rows = pd.DataFrame(removed_records)
    if not exact_duplicate_rows.empty:
        exact_duplicate_rows = exact_duplicate_rows.rename(columns={"__clean_source_row": "Source CSV Row"})
        removed_rows = pd.concat([exact_duplicate_rows, removed_rows], ignore_index=True, sort=False)

    merge_map = pd.DataFrame(merge_records)
    conflicts = pd.DataFrame(conflict_records)
    duplicate_groups = merge_map[merge_map["Action"].isin([
        "Merged duplicate household records",
        "Created one household row from repeated rows",
    ])] if not merge_map.empty else pd.DataFrame()

    metrics = {
        "Original Rows": len(raw_df),
        "Rows After Blank Row Drop": len(working) + len(exact_duplicate_rows),
        "Cleaned Rows": len(cleaned_df),
        "Rows Removed": len(raw_df) - len(cleaned_df),
        "Physical Households": int(merge_map["Physical Household Key"].nunique()) if not merge_map.empty else 0,
        "Household Rows Merged": duplicate_household_rows_merged,
        "Synthetic Households Created": synthetic_households_created,
        "Exact Duplicate Rows Removed": len(exact_duplicate_rows),
        "Repeated Rows Removed": repeat_duplicates_removed,
        "Household Merge Groups": len(duplicate_groups),
        "Household Conflicts": len(conflicts),
    }
    summary = pd.DataFrame({"Metric": list(metrics.keys()), "Value": list(metrics.values())})
    detected_columns = pd.DataFrame([
        {"Role": "Record ID", "Column": record_col or "(none)"},
        {"Role": "Repeat Instrument", "Column": repeat_col or "(none)"},
        {"Role": "Repeat Instance", "Column": instance_col or "(none)"},
        {"Role": "Cluster", "Column": cluster_col or "(none)"},
        {"Role": "Household Number", "Column": household_col or "(none)"},
        {"Role": "Date", "Column": date_col or "(none)"},
    ])
...
def build_cleaner_excel_report(cleaned_df, report):
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        report["summary"].to_excel(writer, sheet_name="Summary", index=False)
        report["detected_columns"].to_excel(writer, sheet_name="Detected Columns", index=False)
        report["merge_map"].to_excel(writer, sheet_name="Household Merge Audit", index=False)
        if not report["removed_rows"].empty:
            report["removed_rows"].to_excel(writer, sheet_name="Removed Redundancy", index=False)
        else:
            pd.DataFrame({"Message": ["No redundant rows removed."]}).to_excel(
                writer, sheet_name="Removed Redundancy", index=False
            )
        if not report["conflicts"].empty:
            report["conflicts"].to_excel(writer, sheet_name="Conflict Review", index=False)
        else:
            pd.DataFrame({"Message": ["No conflicting household values found."]}).to_excel(
                writer, sheet_name="Conflict Review", index=False
            )
        if len(cleaned_df) <= 1_048_000:
...
# ─────────────────────────────────────────────
    ...
    with st.spinner("🔄 Processing files and matching participants..."):
        ...
        # Apply the same dynamic date/examiner filters to each source before matching.
        with st.sidebar:
            st.markdown("### Dynamic Filters")
            st.caption("These filters are applied before duplicate matching.")
            for source_label, source_key in [(label_a, "a"), (label_b, "b")]:
                st.markdown(f"**{source_label}**")
                source_df = pt_a if source_key == "a" else pt_b
                source_date_col = next((c for c in detect_date_columns(source_df) if c in source_df.columns), None)
                if source_date_col:
                    source_dates = parse_date_column(source_df[source_date_col])
                    if source_dates.notna().any():
                        source_min, source_max = source_dates.min().date(), source_dates.max().date()
                        source_range = st.date_input(
                            "Date range", value=(source_min, source_max), min_value=source_min, max_value=source_max,
                            key=f"two_file_date_{source_key}",
                        )
                        if isinstance(source_range, (list, tuple)) and len(source_range) == 2:
                            source_df = source_df[source_dates.dt.date.between(source_range[0], source_range[1]).fillna(False)].copy()
                if "examiner_number" in source_df.columns:
                    source_examiners = sorted(source_df["examiner_number"].dropna().astype(str).unique().tolist())
                    if source_examiners:
                        selected_source_examiners = st.multiselect(
                            "Examiner", source_examiners, default=source_examiners,
                            key=f"two_file_examiner_{source_key}",
                        )
                        if selected_source_examiners:
                            source_df = source_df[source_df["examiner_number"].astype(str).isin(selected_source_examiners)].copy()
                if source_key == "a":
                    pt_a = source_df
                else:
                    pt_b = source_df
    ...
    if not filtered_cols and not field_search.strip() and all_compare_cols:
        filtered_cols = list(all_compare_cols)
        st.info("The clinical preset did not resolve any fields for this file, so Deep Dive is showing all comparable fields.")
    ...
    agreement_totals = agreement_rollup(agreement_df, len(filtered_cols))
    avg_agreement = agreement_totals['Avg Agreement %']
    overall_agreement = agreement_totals['Overall Agreement %']
    ...
    summary_data = {
        'Metric': [
            ...
            'Fields With Usable Data', 'Data Points Assessed', 'Overall Agreement (%)',
        ...
        'Value': [
            ...
            f"{avg_agreement:.1f}%", agreement_totals['Fields With Usable Data'],
            agreement_totals['Data Points Assessed'], f"{overall_agreement:.1f}%",
            low_agree_cnt, len(all_disc_df),
            ', '.join(selected_keys),
    ...
    for col, color, val, lbl in [
        ...
        (c4, "orange" if (len(unmatched_a)+len(unmatched_b)) > 0 else "green",
           ...
           (c5, "green" if overall_agreement >= 80 else "orange" if overall_agreement >= 60 else "red",
               f"{overall_agreement:.1f}%", "Overall Agreement"),
    ...
    with tab_agg_examiner:
        ...
        if drill_choice == label_a:
            if agg_summary_A.empty:
                ...
                if ag_df.empty:
                    st.info("No selected fields have usable values for this examiner.")
                else:
...
# ─────────────────────────────────────────────
    ...
    with st.spinner("🔄 Processing file and splitting groups..."):
        ...
        if date_col:
            def apply_single_file_date_filter(source_df):
                if date_col not in source_df.columns:
                    return source_df
                parsed = parse_date_column(source_df[date_col])
                if date_mode == "Date range" and date_range_choice and len(date_range_choice) == 2:
                    return source_df[parsed.dt.date.between(date_range_choice[0], date_range_choice[1]).fillna(False)]
                if date_mode == "Specific date(s)" and specific_dates_choice:
                    return source_df[parsed.dt.date.isin(specific_dates_choice).fillna(False)]
                return source_df

            if date_mode != "All dates":
                pt_recorded = apply_single_file_date_filter(pt_recorded)
                pt_duplicate = apply_single_file_date_filter(pt_duplicate)
...
# ─────────────────────────────────────────────
    ...
    with st.sidebar:
        ...
        cleaner_upload = st.file_uploader(
            "Upload REDCap CSV", type=["csv"], key="data_cleaner_upload"
        )

    if not cleaner_upload:
        st.markdown('<div style="font-family:Playfair Display,serif;font-size:2.2rem;color:#0f172a;">Data Cleaner</div>', unsafe_allow_html=True)
        st.markdown('<div style="color:#64748b;margin-bottom:20px;">Upload one REDCap CSV to merge duplicate household records, preserve participant rows, and download a clean analysis file.</div>', unsafe_allow_html=True)
        st.info("The cleaner only merges records when both Cluster Code and Household Number match. Rows without both values stay separate.")
        st.stop()
    ...
    try:
        cleaner_raw = load_cleaner_csv(cleaner_upload.getvalue())
    except Exception as exc:
        st.error(f"Could not read the uploaded CSV: {exc}")
        st.stop()

    detected_cleaner = cleaner_detect_columns(cleaner_raw)
    cleaner_columns = list(cleaner_raw.columns)
    no_column = "(not available)"

    def cleaner_column_select(label, detected_name, key):
        options = [no_column] + cleaner_columns
        default_index = options.index(detected_name) if detected_name in options else 0
        picked = st.sidebar.selectbox(label, options, index=default_index, key=key)
        return None if picked == no_column else picked

    with st.sidebar:
        st.markdown("### Match Columns")
        cleaner_record_col = cleaner_column_select("Record ID", detected_cleaner["record_col"], "cleaner_record_col")
        cleaner_repeat_col = cleaner_column_select("Repeat instrument", detected_cleaner["repeat_col"], "cleaner_repeat_col")
        cleaner_instance_col = cleaner_column_select("Repeat instance", detected_cleaner["instance_col"], "cleaner_instance_col")
        cleaner_cluster_col = cleaner_column_select("Cluster", detected_cleaner["cluster_col"], "cleaner_cluster_col")
        cleaner_household_col = cleaner_column_select("Household number", detected_cleaner["household_col"], "cleaner_household_col")
        cleaner_date_col = cleaner_column_select("Date", detected_cleaner["date_col"], "cleaner_date_col")
        cleaner_examiner_col = cleaner_column_select(
            "Examiner", find_col(["Examiner Number", "examiner_number", "Examiner", "Enumerator"], cleaner_columns), "cleaner_examiner_col"
        )

    cleaner_filtered = cleaner_raw.copy()

    def cleaner_apply_filter_preserving_households(source_df, selected_mask):
        """Keep filtered repeated rows and their parent household rows for valid exports."""
        selected_mask = selected_mask.fillna(False)
        if not cleaner_record_col or cleaner_record_col not in source_df.columns:
            return source_df[selected_mask].copy()
        selected_ids = set(
            source_df.loc[selected_mask, cleaner_record_col].map(cleaner_key_value)
        ) - {""}
        if not selected_ids or not cleaner_repeat_col or cleaner_repeat_col not in source_df.columns:
            return source_df[selected_mask].copy()
        repeat_text = source_df[cleaner_repeat_col].astype("string").str.strip().str.casefold()
        household_mask = source_df[cleaner_repeat_col].map(cleaner_is_blank) | repeat_text.isin(["", "nan", "<na>"])
        parent_household_mask = household_mask & source_df[cleaner_record_col].map(cleaner_key_value).isin(selected_ids)
        return source_df[selected_mask | parent_household_mask].copy()

    with st.sidebar:
        st.markdown("### Dynamic Filters")
        if cleaner_date_col:
            cleaner_dates = parse_date_column(cleaner_filtered[cleaner_date_col])
            if cleaner_dates.notna().any():
                date_min, date_max = cleaner_dates.min().date(), cleaner_dates.max().date()
                selected_cleaner_dates = st.date_input(
                    "Date range", value=(date_min, date_max), min_value=date_min, max_value=date_max,
                    key="cleaner_date_range",
                )
                if isinstance(selected_cleaner_dates, (list, tuple)) and len(selected_cleaner_dates) == 2:
                    date_mask = cleaner_dates.dt.date.between(selected_cleaner_dates[0], selected_cleaner_dates[1])
                    cleaner_filtered = cleaner_apply_filter_preserving_households(cleaner_filtered, date_mask)
        if cleaner_examiner_col:
            cleaner_examiners = sorted(
                cleaner_filtered[cleaner_examiner_col].dropna().astype(str).unique().tolist()
            )
            if cleaner_examiners:
                selected_cleaner_examiners = st.multiselect(
                    "Examiner", cleaner_examiners, default=cleaner_examiners, key="cleaner_examiners"
                )
                if selected_cleaner_examiners:
                    examiner_mask = cleaner_filtered[cleaner_examiner_col].astype(str).isin(selected_cleaner_examiners)
                    cleaner_filtered = cleaner_apply_filter_preserving_households(cleaner_filtered, examiner_mask)

        st.markdown("### Cleaning Rules")
        cleaner_trim_text = st.checkbox("Trim blank spaces", value=True, key="cleaner_trim_text")
        cleaner_exact_rows = st.checkbox("Remove exact duplicate rows", value=True, key="cleaner_exact_rows")
        cleaner_instances = st.checkbox("Renumber repeat instances", value=True, key="cleaner_instances")
        cleaner_strategy = st.selectbox(
            "Household record to keep",
            ["Most complete household row", "Latest date, then most complete", "Earliest row in file"],
            key="cleaner_strategy",
        )

    if not cleaner_record_col or not cleaner_cluster_col or not cleaner_household_col:
        st.error("Select Record ID, Cluster, and Household Number columns before running the cleaner.")
        st.stop()

    cleaned_data, cleaner_report = clean_redundant_households(
        cleaner_filtered,
        record_col=cleaner_record_col,
        repeat_col=cleaner_repeat_col,
        instance_col=cleaner_instance_col,
        cluster_col=cleaner_cluster_col,
        household_col=cleaner_household_col,
        date_col=cleaner_date_col,
        trim_text=cleaner_trim_text,
        remove_exact_rows=cleaner_exact_rows,
        recompute_instances=cleaner_instances,
        household_strategy=cleaner_strategy,
    )

    st.markdown('<div style="font-family:Playfair Display,serif;font-size:1.9rem;color:#0f172a;">Data Cleaner</div>', unsafe_allow_html=True)
    st.caption("A physical household is identified by the combination of Cluster Code and Household Number. The first retained household Record ID becomes the canonical record, and all retained repeat rows are attached to it.")

    cleaner_metrics = cleaner_report["metrics"]
    metric_cols = st.columns(6)
    metric_cols[0].metric("Input rows", f"{cleaner_metrics.get('Original Rows', 0):,}")
    metric_cols[1].metric("Clean rows", f"{cleaner_metrics.get('Cleaned Rows', 0):,}")
    metric_cols[2].metric("Rows removed", f"{cleaner_metrics.get('Rows Removed', 0):,}")
    metric_cols[3].metric("Households", f"{cleaner_metrics.get('Physical Households', 0):,}")
    metric_cols[4].metric("Household merges", f"{cleaner_metrics.get('Household Merge Groups', 0):,}")
    metric_cols[5].metric("Conflicts to review", f"{cleaner_metrics.get('Household Conflicts', 0):,}")

    if cleaner_metrics.get("Household Conflicts", 0):
        st.warning("Some duplicate household records had different non-empty values. The selected canonical row was kept; review the Conflict Review tab before using the export.")

    cleaner_tabs = st.tabs(["Cleaned Data", "Household Merge Audit", "Removed Redundancy", "Conflict Review", "Download"])
    with cleaner_tabs[0]:
        st.dataframe(cleaned_data, use_container_width=True, hide_index=True, height=520)
    with cleaner_tabs[1]:
        st.dataframe(cleaner_report["merge_map"], use_container_width=True, hide_index=True, height=520)
    with cleaner_tabs[2]:
        if cleaner_report["removed_rows"].empty:
            st.success("No exact or repeated duplicate rows were removed.")
        else:
            st.dataframe(cleaner_report["removed_rows"], use_container_width=True, hide_index=True, height=520)
    with cleaner_tabs[3]:
        if cleaner_report["conflicts"].empty:
            st.success("No conflicting non-empty household values were found.")
        else:
            st.dataframe(cleaner_report["conflicts"], use_container_width=True, hide_index=True, height=520)
    with cleaner_tabs[4]:
        cleaned_csv = cleaned_data.to_csv(index=False).encode("utf-8-sig")
        cleaner_excel = build_cleaner_excel_report(cleaned_data, cleaner_report)
        dl_a, dl_b = st.columns(2)
        with dl_a:
            st.download_button(
                "Download cleaned CSV", cleaned_csv, "cleaned_household_data.csv", "text/csv", use_container_width=True
            )
        with dl_b:
            st.download_button(
                "Download cleaning workbook", cleaner_excel, "household_cleaning_audit.xlsx",
                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True,
            )
        st.dataframe(cleaner_report["summary"], use_container_width=True, hide_index=True)
...
# ─────────────────────────────────────────────
    ...
    col_enum     = find_col(["Enumerator Username/number", "Enumerator", "examiner_number", "Examiner Number", "Examiner", "enumerator"], raw.columns)
    ...
    with st.sidebar:
        ...
        staff_label = "Examiner" if col_enum and "examiner" in col_enum.lower() else "Enumerator"
        sel_enums = st.multiselect(staff_label, enums, default=enums) if enums else []
