"""
Unified Oral Health Dashboard
Combines:
  - Comprehensive Survey Analytics (app timer)
  - Duplicate Examination Comparator (dashboard v9 refined)
...
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="OralHealth Analytics Pro + Duplicate Comparator",
    page_icon="🦷",
    layout="wide",
...
# ─────────────────────────────────────────────
# SHARED HELPERS (AGE GROUP CORRUPTION, ETC.)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
with st.sidebar:
    st.markdown('<div style="padding:14px 0 18px;"><div style="font-family:Playfair Display,serif;font-size:1.3rem;color:white;">🦷 OralHealth Pro + Comparator</div><div style="font-size:.7rem;color:#64748b;margin-top:2px;">Unified Analytics</div></div>',unsafe_allow_html=True)

    mode = st.radio("Mode", ["📊 Survey Analysis", "🔁 Duplicate Comparator"], index=0)

    if mode == "📊 Survey Analysis":
        uploaded = st.file_uploader("Upload Survey CSV", type=["csv"], label_visibility="collapsed")
        st.markdown("---")
        # Survey filters will be shown in main if file loaded
    else:  # Comparator
...
if mode == "📊 Survey Analysis":
    ...
    # ── SIDEBAR FILTERS ──
    # (already rendered in sidebar, but we need to apply them here)
    # We'll re-use the same logic as in the original survey dashboard
    with st.sidebar:
        st.markdown("### Filters")
    ...
    with tabs[1]:
        ...
        if len(hh)==0:
            ...
            # Household status analysis (as in original)
...
else:  # ─── DUPLICATE COMPARATOR MODE ───
