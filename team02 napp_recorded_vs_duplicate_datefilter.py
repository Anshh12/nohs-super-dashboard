...
def build_participant_df(df, label, examiner_present=True):
    """
    Extract participant rows from the full REDCap export.
    If examiner_present=True, keep only rows with non-null examiner_number (recorded).
    If False, keep only rows with null examiner_number (duplicate).
    """
    ...
    if 'examiner_number' in pt.columns:
        if examiner_present:
            pt = pt[pt['examiner_number'].notna()]
        else:
            pt = pt[pt['examiner_number'].isna()]
    else:
        if examiner_present:
            pt = pt
        else:
...
# ─────────────────────────────────────────────
# 📅 DATE FILTER  (sidebar UI, applied to the "Recorded" side before matching)
# ─────────────────────────────────────────────
...
with st.sidebar:
    ...
    if not date_candidates:
        st.caption("No date-like column found in this file — nothing to filter on.")
    else:
        ...
        if date_col != "(none)":
            parsed_all = parse_date_column(df[date_col])
            valid_dates = parsed_all.dropna()
            if valid_dates.empty:
                st.warning(f"Couldn't parse any valid dates from '{date_col}'.")
                date_col = None
            else:
                min_d, max_d = valid_dates.dt.date.min(), valid_dates.dt.date.max()
                date_mode = st.radio("Filter mode", ["All dates", "Date range", "Specific date(s)"], index=0)
                if date_mode == "Date range":
                    date_range_choice = st.date_input(
                    ...
                    specific_dates_choice = st.multiselect(
...
with st.spinner("🔄 Processing file and splitting groups..."):
    pt_recorded = build_participant_df(df, label_a, examiner_present=True)
    pt_duplicate = build_participant_df(df, label_b, examiner_present=False)

    # Apply the date filter to the Recorded side (that's where the date lives,
    # since the blank-examiner "duplicate" rows don't carry their own date in
    # most REDCap exports of this type). Matching then naturally restricts the
    # Duplicate side too, since only recorded rows within the filter survive.
