...
def parse_date_column(series):
    """Parse date columns robustly for exported labels and typed CSV data."""
...
def clean_cluster_val(v):
    """Display numeric cluster IDs without a trailing .0."""
    if pd.isna(v):
        return "(missing)"
    text = str(v).strip()
...
def extract_participants(df):
    """Extract participant rows and attach household identifiers to them."""
    hh_cols = [c for c in ['record_id', 'cluster_code', 'household_number'] if c in df.columns]
    ...
    for col in ['cluster_code', 'household_number']:
        if col in pt.columns:
            pt = pt.drop(columns=[col])
