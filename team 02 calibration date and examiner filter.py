...
def normalize_export_columns(df):
    ...
    column_map = {
        # Labelled-export headers -> internal names
        ...
        # Raw-export headers -> internal names (e.g. 'redcap_repeat_instrument',
        # 'examiner_suffix', 'age', 'sex' as used in this project's raw CSV export)
...
def detect_date_columns(df):
    """Any column whose name contains 'date' is offered as a candidate for the
    date filter - works regardless of which project/export this is."""
    return [c for c in df.columns if 'date' in c.lower()]

def parse_date_column(series):
    """Best-effort parse to pandas datetime, tolerant of timestamp strings
    like '2026-08-31 11:02:54' as well as plain dates."""
...
def extract_participants(df):
    """
    Extract participant-level rows from the full REDCap export.
    Handles both simple and repeating-instrument structures.
    Does NOT filter by examiner number – that is done later.
    """
...
def get_unmatched(pt, other_pt, keys):
    """Return participants in pt that have no match in other_pt based on keys."""
...
with st.sidebar:
    ...
    # These widgets will be populated after loading the file (dynamic)
    # We'll store them in session_state or use placeholders.
    # We'll create them later after we read the file, but we need to place them here.
    # We'll use a placeholder for now and update in main.
    # We'll store references to these widgets in session_state.
    # Better: we can define them after file is loaded, but sidebar is rendered before main.
    # So we will define them inside the main block using st.sidebar.xxx.
    # We'll just put a placeholder text and then we'll create actual widgets in main.
    # Actually we can create them here but they need to access data, so we'll do it in main.
    # We'll store a placeholder for now.
...
# Now that we have data, we populate the examiner selection widgets in the sidebar
...
# Compute masks for recorded and duplicate groups
...
recorded_mask = pt_all['examiner_number'].notna() & ~pt_all['examiner_number'].astype(str).isin(selected_examiners)

# Rows that are neither (null not included and not selected) are excluded
excluded_mask = ~duplicate_mask & ~recorded_mask
excluded_count = excluded_mask.sum()

if excluded_count > 0:
    st.sidebar.warning(f"{excluded_count} rows have examiner numbers not selected and are not empty — they will be ignored (neither recorded nor duplicate).")

# Warn if duplicate_mask is empty
if duplicate_mask.sum() == 0:
    st.sidebar.error("No rows match the duplicate definition. Please select at least one examiner number or include empty.")
    # We'll still allow to proceed but duplicate group empty.

# ─── DATE FILTER (sidebar) ───
# We need to re-create the date filter widgets in sidebar, but they were already defined? We'll define them here.
# We'll put them after the examiner selection.
st.sidebar.divider()
...
with st.spinner("🔄 Processing file and splitting groups..."):
    # Start with recorded and duplicate from the masks
    pt_recorded = pt_all[recorded_mask].copy()
    pt_duplicate = pt_all[duplicate_mask].copy()

    # Apply date filter to recorded group (only recorded rows)
    ...
    # Detect corrupted age_group values
...
# Build summary data, including examiner selection details
summary_data = {
    'Metric': [
...
# TABS
# ─────────────────────────────────────────────
tab_summary, tab_matched, tab_unmatched, tab_field, tab_deepdive, tab_discrep, tab_export = st.tabs([
...
with tab_export:
    ...
    st.download_button(
        "📥 Download Full Excel Report (All Sheets + Per-Category)",
